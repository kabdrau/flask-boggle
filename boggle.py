...
class Boggle():
    ...
    def make_board(self, N):
        ...
        for y in range(N):
            row = [choice(string.ascii_uppercase) for i in range(N)]
            board.append(row)

        return board

    def check_valid_word(self, board, word, N):
        """Check if a word is a valid word in the dictionary and/or the boggle board"""

        word_exists = word in self.words
        valid_word = self.find(board, word.upper(), N)
    ...
    def find_from(self, board, word, y, x, seen, N):
